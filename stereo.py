...
def fastDetect(img1L, img2L):
    ...
    trackPoints2, st, err = cv2.calcOpticalFlowPyrLK(
        img1L, img2L, trackPoints1, None, flags=cv2.MOTION_AFFINE, **lk_params
    )
...
def featureEliminator(
    ...
    for i in range(kps1L.shape[0]):
        ...
        if (
                disparityMinThres < T1Disparity < disparityMaxThres
                    and disparityMinThres < T2Disparity < disparityMaxThres
        ):
...
def minimizeReprojection(
    dof, d2dPoints1, d2dPoints2, d3dPoints1, d3dPoints2, w2cMatrix
):
...
def estimateOdometry(img1, img2, Proj1, Proj2, debug=False):
    # img1 = [img1L,img1R]
    # img2 = [img2L,img2R]
    ...
    (
        ...
        img1_disparity, img2_disparity, trackPoints1_KLT_L, trackPoints2_KLT_L
    )

    if debug:
        saveDebugImg(img2[L], 1, "TrackedPoints2", trackPoints2L_3d, color=(0, 255, 0))

    # 3d triangulation
    numPoints = trackPoints1L_3d.shape[0]
    d3dPointsT1 = triangulate3D(
        trackPoints1L_3d, trackPoints1R_3d, numPoints, Proj1, Proj2
    )
    d3dPointsT2 = triangulate3D(
        trackPoints2L_3d, trackPoints2R_3d, numPoints, Proj1, Proj2
    )
    ...
    if debug:
        saveDebugImg(img2[L], 1, "CliquePoints", trackedPoints2L, color=(0, 255, 0))

    if trackedPoints1L.shape[0] >= 6:
        dSeed = np.zeros(6)

        optRes = least_squares(
            minimizeReprojection,
            dSeed,
            method="lm",
            max_nfev=2000,
            args=(
...
if __name__ == "__main__":
    P0L, P0R = getGt("./dataset/sequences/01/calib.txt")
    P1L, P1R = getGt("./dataset/sequences/01/calib.txt")  # same calibration file
    # print(P0)
    # print(P1)
    ...
    for i in tqdm(range(1, 4)):
        path1L = "./dataset/sequences/01/image_0/" + str(i - 1).zfill(6) + ".png"
        path1R = "./dataset/sequences/01/image_1/" + str(i - 1).zfill(6) + ".png"
        path2L = "./dataset/sequences/01/image_0/" + str(i).zfill(6) + ".png"
        path2R = "./dataset/sequences/01/image_1/" + str(i).zfill(6) + ".png"
        ...
        # print("Cost : ",cost)
        print(
            "X : ", transGlobal[0], "  Y : ", transGlobal[1], "  Z : ", transGlobal[2]
        )
        f.write(
            "0.0 0.0 0.0 "
            + str(transGlobal[0])
